    ...
    <div className="p-10 bg-[#F8FAFC] min-h-screen font-sans text-slate-800">
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em]">Patient Chart Archive</p>
        </div>
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-8 duration-500">
          ...
          <div className="bg-white p-6 rounded-[1.5rem] border border-slate-100 shadow-sm">
            <div className="flex justify-between items-center">
              ...
              <button 
                onClick={() => { setActivePatient(null); setHistory([]); setSearchTerm(""); }} 
            ...
            <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-slate-50">
              <div className="bg-slate-50 p-4 rounded-2xl">
                <div className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Blood Pressure</div>
                <div className="text-xl font-black mt-1 text-slate-700">{triageInfo?.blood_pressure || "--"}</div>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl">
                <div className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Heart Rate</div>
                <div className="text-xl font-black mt-1 text-slate-700">{triageInfo?.heart_rate || triageInfo?.pulse_rate || "--"}</div>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl">
                <div className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Weight</div>
                <div className="text-xl font-black mt-1 text-slate-700">{triageInfo?.weight || activePatient.weight || "--"} <span className="text-xs text-slate-400">kg</span></div>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl">
                <div className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Height</div>
                <div className="text-xl font-black mt-1 text-slate-700">{triageInfo?.height || activePatient.height || "--"} <span className="text-xs text-slate-400">cm</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  {history.map((h) => (
                    <div key={h.id} className="p-5 bg-slate-50 rounded-2xl flex justify-between items-start group hover:bg-white hover:shadow-md hover:border-emerald-100 border border-transparent transition-all cursor-default">
                      <div>
                        ...
                        <div className="font-black text-slate-800 text-sm uppercase">{h.diagnosis || h.assessment || "Clinical Note"}</div>
                        <div className="text-xs font-medium text-slate-500 mt-2 italic">"{h.subjective_notes || h.chief_complaint || "No notes recorded."}"</div>
                      </div>
                      <div className="text-slate-300 group-hover:text-emerald-500 transition-colors"><ChevronRight size={18} /></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Latest Triage</h3>
                {triageInfo ? (
                  <div className="space-y-4">
                    <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                        <div className="text-[9px] font-bold text-emerald-600 uppercase mb-1">Chief Complaint</div>
                        <div className="text-sm font-bold text-slate-700 leading-tight">"{triageInfo.chief_complaint}"</div>
                    </div>
                    {triageInfo.notes && (
                        <div className="text-xs text-slate-500 font-medium leading-relaxed">
                            <span className="font-bold text-slate-700">Notes:</span> {triageInfo.notes}
                        </div>
                    )}
                  </div>
                ) : (
                  <div className="text-xs text-slate-400 font-bold italic">No active triage session.</div>
                )}
              </div>

              <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Coverage</h3>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <div className="text-[9px] font-bold text-slate-400 uppercase mb-1">PhilHealth ID</div>
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in border border-white/10">
            <div className="p-6 border-b flex justify-between items-center bg-gray-50">
              <h4 className="font-black text-slate-400 text-[10px] uppercase tracking-widest">Scan Patient QR</h4>
              <button onClick={() => setShowScanner(false)} className="text-slate-300 hover:text-red-500 transition-colors"><X size={20}/></button>
            </div>
