...
const ReferralCenter = () => {
          ...
          setReferrals((prev) =>
            prev.map((ref) => {
              if (ref.ambulances && ref.ambulances.id === payload.new.id) {
                return {
                  ...ref,
                  ambulances: { ...ref.ambulances, ...payload.new },
                };
              }
              return ref;
            }),
  ...
  const calculateLiveETA = (lat, lon) => {
    ...
    const a =
      ...
      Math.cos(lat * (Math.PI / 180)) *
        Math.cos(HOSPITAL_LOCATION.lat * (Math.PI / 180)) *
        Math.sin(dLon / 2) *
        Math.sin(dLon / 2);
    const time = Math.round(
      ((R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a))) / 30) * 60,
    );
  ...
  const displayData = referrals.filter((ref) => {
    const fullName =
      `${ref.users?.first_name} ${ref.users?.last_name}`.toLowerCase();
    const matchesSearch =
      fullName.includes(searchTerm.toLowerCase()) ||
      ref.users?.medical_id?.toLowerCase().includes(searchTerm.toLowerCase());
    return showArchive
      ? matchesSearch && ref.status !== "PENDING"
      : matchesSearch && ref.status === "PENDING";
  });

  return (
    <div className="p-12 bg-[#F1F5F9] min-h-screen relative overflow-hidden font-sans">
      {/* HEADER SECTION */}
      ...
      <div className="grid grid-cols-12 px-10 mb-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">
        ...
        <div className="col-span-2 text-center">
          {showArchive ? "Decision Status" : "Estimated Arrival"}
        </div>
          ...
          >
            ...
            <div
              className={`absolute left-0 top-8 bottom-8 w-1.5 rounded-r-full transition-all ${
                ref.ai_priority_score >= 0.8
                  ? "bg-rose-500 shadow-[2px_0_10px_rgba(244,63,94,0.4)]"
                  : "bg-emerald-500"
              }`}
            />
            ...
            <div className="col-span-3 flex items-center gap-5 pl-10">
              <div className="w-14 h-14 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center font-black text-xs border border-slate-100 uppercase transition-all group-hover:bg-slate-100 group-hover:text-slate-600">
                {ref.users?.first_name?.[0]}
                {ref.users?.last_name?.[0]}
              </div>
              <div>
            ...
            <div className="col-span-2 flex justify-center">
              <span
                className={`px-5 py-2 rounded-xl text-[9px] font-black border-2 transition-all ${
                  ref.ai_priority_score >= 0.8
              ...
              {showArchive ? (
                ...
                <div className="animate-in fade-in duration-700">
                  <p className="text-lg font-black text-slate-800 leading-none tabular-nums">
                    {calculateLiveETA(
                      ref.ambulances?.latitude,
                      ref.ambulances?.longitude,
                    )}
                  </p>
      ...
      {selectedReferral && (
        <ReferralModal
          referral={
            referrals.find((r) => r.id === selectedReferral.id) ||
            selectedReferral
          }
          isLog={showArchive}
