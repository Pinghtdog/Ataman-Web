...
const Consultations = () => {
  ...
  useEffect(() => {
    ...
    // 2. Listen for ANY changes to the 'bookings' table (Physical Intakes)
    ...
    // 3. Listen for ANY changes to the 'telemed_sessions' table (Video Consults)
    ...
    // 4. Cleanup listeners when leaving the page
    return () => {
  ...
  const initializeConsole = async () => {
    ...
    try {
      ...
      if (staffData) {
        ...
        // 1. Fetch Physical Intakes (Safely build query to avoid 'undefined' error)
        let bookingsQuery = supabase
          ...
          .eq("status", "pending");
        // Only append doctor ID filter if doctorId exists
        if (doctorId) {
        ...
        // 2. Fetch Telemedicine Sessions (Only if doctorId exists)
        let telemedsData = [];
        if (doctorId) {
          const { data: tData, error: telemedError } = await supabase
            .from("telemed_sessions")
            // FIX: Removed 'created_at', added 'started_at', simplified 'users(*)'
            .select(
          ...
          combined.push(
            ...telemedsData.map((t) => {
              ...
              return {
                ...
                original_id: t.id,
                // FIX: Swapped 'created_at' for 'started_at' as the fallback
                time:
      ...
      {!activePatient && (
        <div className="animate-in slide-in-from-top-4 duration-500 flex-1 flex flex-col min-h-0">
          ...
          <div className="max-w-md mb-8">
            <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3 focus-within:ring-2 ring-emerald-500/20 transition-all">
