...
const ServiceAndFacilities = () => {
  ...
  const fetchData = async () => {
    // 1. Fetch Resources (Equipment & Supplies)
    ...
    // 2. Fetch Real-time Ward Occupancy from the 'beds' table
  ...
  useEffect(() => {
    fetchData();
    // Real-time listeners for resources AND beds
  ...
  const equipment = resources.filter(r => r.resource_category === 'equipment' && r.resource_type.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const subCategories = ['All', 'Medications', 'PPE', 'First Aid', 'Nutritional', 'Sanitation'];

  if (loading) return <div className="h-screen flex items-center justify-center bg-white font-mono text-emerald-600 animate-pulse">Synchronizing Command Center...</div>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm overflow-x-auto no-scrollbar">
            ...
            <div className="flex gap-4">
              {resources.filter((v, i, a) => a.findIndex(t => t.departments?.id === v.departments?.id) === i).map(r => (
                <div key={r.departments?.id} className="min-w-[130px] bg-slate-50 p-4 rounded-2xl border border-slate-100 group hover:border-emerald-200 transition-all">
        ...
        {criticalItems.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-[10px] font-black text-rose-500 uppercase tracking-widest flex items-center gap-2">
              <span className="w-2 h-2 bg-rose-500 rounded-full animate-ping" /> Global Shortage Warnings
            </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col h-[520px] shadow-sm">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {equipment.map(item => (
                <div key={item.id} className="p-5 rounded-2xl border border-slate-50 bg-white shadow-sm hover:border-emerald-100 transition-all">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${item.status === 'ONLINE' ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-slate-300'}`} />
                      <div>
                        <p className="text-xs font-black text-slate-800 uppercase leading-none mb-1">{item.resource_type}</p>
                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{item.sub_text || 'Active Asset'}</p>
                      </div>
                    </div>
                    <button 
                    ...
                    >
                      <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all duration-300 shadow-sm ${item.status === 'ONLINE' ? 'right-0.5' : 'left-0.5'}`} />
                    </button>
                  </div>
                  <div className="flex justify-between items-center pt-3 border-t border-slate-50">
                    <span className="text-[10px] font-black text-slate-700 uppercase tracking-tighter">Usage: {item.current_occupied || 0} / {item.total_capacity} Units</span>
                    <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${item.status === 'ONLINE' ? 'text-emerald-600 bg-emerald-50' : 'text-slate-400 bg-slate-50'}`}>
                      {item.status}
                    </span>
                  </div>
                </div>
                    ...
                    <tr key={item.id} className="group hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-4">
                        <span className={`text-[8px] font-black uppercase px-2 py-1 rounded border ${
                          item.status === 'AVAILABLE' ? 'text-emerald-600 bg-emerald-50 border-emerald-100' : 'text-rose-600 bg-rose-50 border-rose-100'
                        }`}>
                          {item.status}
                        </span>
                      </td>
