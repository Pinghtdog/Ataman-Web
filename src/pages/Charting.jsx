...
const Charting = () => {
  ...
  const handleSearch = async (e, directTerm = null) => {
    ...
    const { data } = await supabase
      ...
      .select("*")
      .or(
        `first_name.ilike.%${term}%,last_name.ilike.%${term}%,philhealth_id.eq.${term},id.eq.${term}`,
      );

    if (data?.length === 1) selectPatient(data[0]);
    else if (data?.length > 1) setSearchResults(data);
    else alert("No medical records located.");
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {recentPatients.map((p) => (
              <div
                ...
                className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:border-emerald-500 cursor-pointer flex justify-between items-center group transition-all"
              >
                <div className="flex items-center gap-4">
                  {/* Fixed Crash: Added check for first_name existence */}
                  <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-300 border border-slate-100 font-bold group-hover:text-emerald-500">
                    {p.first_name?.[0] || "?"}
                  </div>
                  <div>
