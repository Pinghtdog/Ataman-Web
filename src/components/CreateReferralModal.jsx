...
const CreateReferralModal = ({
  ...
  const loadFacilityDetails = async (facility) => {
    try {
      ...
      const wardMap = bedsRes.data?.reduce((acc, bed) => {
        if (!acc[bed.ward_type])
          acc[bed.ward_type] = { total: 0, available: 0 };
        acc[bed.ward_type].total++;
        if (bed.status?.toLowerCase() === "available")
          acc[bed.ward_type].available++;
        return acc;
      }, {});

      setFacilityWards(
        Object.entries(wardMap || {}).map(([type, counts]) => ({
          type,
          ...counts,
        })),
      );
  ...
  const handleFinalize = async (e) => {
    ...
    if (
      ...
      return alert(
        "Incomplete Protocol: Verify Subject, Officer, and Documents.",
      );
    }
    setLoading(true);
    try {
      ...
      >
        ...
        <div className="px-8 py-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-base font-black uppercase italic tracking-tighter leading-none text-slate-800">
                Discovery Hub
              </h2>
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-xl hover:bg-rose-50 hover:text-rose-500 transition-all flex items-center justify-center text-slate-300"
          >
            ✕
          </button>
        </div>
            ...
            <div className="p-8 animate-in fade-in duration-500">
              <div className="flex flex-col items-center mb-8">
                <input
                  ...
                  className="w-full max-w-md p-4 bg-white border border-slate-200 rounded-xl text-[10px] font-bold outline-none focus:ring-2 focus:ring-emerald-500/10 transition-all uppercase tracking-widest shadow-sm"
              ...
              <div className="grid grid-cols-4 gap-4">
                {facilities
                  ...
                  .map((f) => (
                    <button
                      key={f.id}
                      onClick={() => loadFacilityDetails(f)}
                      className="bg-white p-5 rounded-2xl border border-slate-100 hover:border-emerald-500 hover:shadow-lg transition-all text-left group"
                    >
                      <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-xl mb-3 border border-slate-100 group-hover:bg-emerald-50 transition-colors">
                        🏥
                      </div>
                      <h4 className="font-black text-[10px] uppercase tracking-tight text-slate-800 leading-tight line-clamp-2 h-8">
                        {f.name}
                      </h4>
                    </button>
                  ))}
            ...
            <div className="animate-in slide-in-from-right-4 duration-500 flex h-full">
              ...
              <div className="w-[30%] p-6 border-r border-slate-100 bg-white space-y-4">
                <button
                  ...
                  className="text-[8px] font-black text-slate-400 uppercase tracking-widest hover:text-emerald-600 transition-colors"
                >
                  ← Back to Registry
                </button>
                ...
                <div>
                  <h3 className="text-lg font-black uppercase italic tracking-tighter leading-tight text-slate-800 mb-2">
                    {selectedFacility.name}
                  </h3>
                  <button
                    onClick={() => setView("form")}
                    className="w-full bg-slate-900 text-white py-3 rounded-xl font-black text-[9px] uppercase tracking-widest shadow-lg hover:bg-emerald-600 transition-all active:scale-95"
                  >
                      ...
                      {facilityWards.map((w) => (
                        <div key={w.type} className="space-y-1.5">
                          <div className="flex justify-between font-black text-[8px] uppercase">
                            ...
                            >
                              {w.available}/{w.total} Beds
                            </span>
                          </div>
                          <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full transition-all duration-1000 ${w.available > 0 ? "bg-emerald-500" : "bg-rose-500"}`}
                              style={{
                                width: `${(w.available / w.total) * 100}%`,
                              }}
                            />
                          </div>
                        </div>
                    ...
                    <div className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                      ...
                      <div className="space-y-2">
                        {facilityDepts.slice(0, 5).map((d) => (
                          <div
                            key={d.id}
                            className="p-2 bg-slate-50 rounded-lg flex items-center justify-between border border-slate-100/50"
                          >
                    ...
                    <div className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                      ...
                      <div className="space-y-2">
                        {facilityEquipment.slice(0, 5).map((r) => (
                          <div
                            key={r.id}
                            className="flex justify-between items-center p-2 bg-slate-50 rounded-lg border border-slate-100/50"
                          >
                            <span className="text-[8px] font-black text-slate-600 uppercase tracking-tight">
                              {r.resource_type}
                            </span>
                            <div className="px-2 py-0.5 rounded bg-emerald-50 border border-emerald-100 text-emerald-600 text-[7px] font-black">
                              READY
                            </div>
                          </div>
                        ))}
          ...
          {view === "form" && (
            <div className="p-8 h-full flex flex-col animate-in zoom-in-95 duration-300">
              <button
                ...
                className="mb-4 text-[8px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-2"
              >
              ...
              <form
                ...
                className="bg-white flex-1 p-8 rounded-[2rem] border border-slate-200 shadow-xl overflow-visible flex flex-col justify-between"
              >
                <div className="grid grid-cols-2 gap-x-8 gap-y-5">
                    ...
                    {!selectedPatient ? (
                      <div className="relative">
                        <input
                          ...
                          className="w-full bg-slate-50 p-3 rounded-xl font-bold text-[10px] outline-none border border-slate-100 focus:border-emerald-500"
                          value={patientSearch}
                          onChange={(e) => {
                        ...
                        <AnimatePresence>
                          {isPatientListOpen && patientSearch.length > 0 && (
                            <motion.div
                              ...
                              exit={{ opacity: 0 }}
                              className="absolute top-full left-0 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-2xl z-[1001] max-h-40 overflow-y-auto custom-scrollbar"
                            >
                              {patients
                                .filter((p) =>
                                ...
                                .map((p) => (
                                  <button
                                    key={p.id}
                                    type="button"
                                    className="w-full p-3 text-left hover:bg-slate-50 text-[10px] font-bold border-b border-slate-50 last:border-none"
                                    onClick={() => {
                      ...
                      <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl flex justify-between items-center">
                        ...
                        <button
                          ...
                          className="text-[8px] font-black text-rose-500 underline uppercase"
                        >
                          CHANGE
                        </button>
                      </div>
                  ...
                  <div className="space-y-2 relative">
                    ...
                    {!selectedStaff ? (
                            ...
                            >
                              {staff
                                ...
                                .map((s) => (
                                  ...
                                  >
                                    {s.name}{" "}
                                    <span className="text-[8px] text-slate-400 ml-1 uppercase">
                                      [{s.role}]
                                    </span>
                                  </button>
                                ))}
                      ...
                      <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl flex justify-between items-center">
                        <span className="text-[9px] font-black uppercase text-blue-800">
                          {selectedStaff.name}
                        </span>
                        <button
                          type="button"
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      required
                      className="w-full bg-slate-50 p-3 rounded-xl font-black text-[10px] border border-slate-100 outline-none"
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative border border-slate-200 border-dashed rounded-xl p-3 h-[42px] flex items-center justify-center bg-slate-50/50">
                      <input
                        ...
                        onChange={(e) => setReferralFile(e.target.files[0])}
                      />
                      <p className="text-[8px] font-black uppercase text-slate-400 truncate px-2">
                        {referralFile
                          ? referralFile.name
                          : "Attach Referral Slip*"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="col-span-2 space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-50 p-4 rounded-xl font-bold text-[10px] border border-slate-100 outline-none resize-none focus:border-emerald-500"
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-slate-900 text-white py-4 mt-6 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl hover:bg-emerald-600 transition-all disabled:opacity-50"
                >
                  {loading
