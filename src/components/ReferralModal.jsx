...
const ReferralModal = ({
  ...
  const [serviceStream, setServiceStream] = useState(
    referral.service_stream || "OUTPATIENT",
  );
  const [selectedWard, setSelectedWard] = useState(
    referral.assigned_unit || "",
  );
  const [selectedBed, setSelectedBed] = useState(
    referral.assigned_bed_id || "",
  );
>>>>>>> b0420092242b0650ed0d82d71e4793cd88371f53
...
=======
          ...
          supabase
            ...
            .select("*")
            .eq("resource_category", "equipment")
            .eq("facility_id", fid),
          supabase
            .from("facilities")
            .select("name")
            .eq("id", initialReferral.origin_facility_id)
            .single(),
          supabase
            .from("beds")
...
=======
        console.error(
          "❌ ERROR: facilityId is null. Staff record might not be loaded.",
        );
>>>>>>> b0420092242b0650ed0d82d71e4793cd88371f53
...
=======
      if (normalizedStream === "DIAGNOSTIC" && selectedWard) {
        ...
        const cleanResourceName = selectedWard.split(" — ")[0].trim();
        console.log(
          `🔍 [LOOKUP] Searching facility_resources for: "${cleanResourceName}" in Facility: ${facilityId}`,
        );
        ...
        if (lookupError || !resource) {
          console.error(
            "❌ [LOOKUP FAILED]:",
            lookupError?.message || "No record found",
          );
          throw new Error(
            `Could not find a resource record named "${cleanResourceName}"`,
          );
>>>>>>> b0420092242b0650ed0d82d71e4793cd88371f53
        }
...
=======
        const newStatus =
          newOccupancy >= resource.total_capacity ? "OFFLINE" : "ONLINE";

        console.log(
          `📤 [UPDATING RESOURCE]: ID ${resource.id} | New Occupancy: ${newOccupancy} | New Status: ${newStatus}`,
        );
...
=======
          console.warn(
            "⚠️ [RLS WARNING]: Update successful but 0 rows changed. Check RLS policies!",
          );
>>>>>>> b0420092242b0650ed0d82d71e4793cd88371f53
        } else {
...
=======
      if (normalizedStream === "INPATIENT" && selectedBed) {
        console.log(
          `🛏️ [UPDATING BED]: ID ${selectedBed} for Patient ${targetPatientId}`,
        );
        const { error: bedError } = await supabase
          .from("beds")
          .update({ status: "occupied", patient_id: targetPatientId })
...
=======
        ...
        .update({
          ...
          assigned_unit: selectedWard,
          assigned_bed_id:
            normalizedStream === "INPATIENT" ? selectedBed : null,
          destination_facility_id: facilityId,
        })
        .eq("id", referral.id);
>>>>>>> b0420092242b0650ed0d82d71e4793cd88371f53
...
=======
        ...
        <div className="w-[45%] relative bg-slate-50 border-r border-slate-100 overflow-hidden">
          <div
            className={`absolute inset-0 transition-all duration-500 ${leftPanelView === "map" ? "opacity-100" : "opacity-0 scale-95 pointer-events-none"}`}
          >
            <Map height={650} center={HOSPITAL_LOCATION} defaultZoom={14}>
              <Marker width={35} anchor={HOSPITAL_LOCATION} color="#0D9488" />
              {referral.ambulances?.latitude && (
            ...
            <div className="absolute bottom-6 left-6 right-6 bg-white/95 p-5 rounded-[2rem] shadow-xl border border-slate-100 backdrop-blur-md">
              ...
              <h4 className="text-sm font-black text-slate-800 uppercase tracking-tight">
                {originHospital?.name || "Referring Facility"}
              </h4>
          ...
          <div
            className={`absolute inset-0 bg-slate-100 transition-all duration-500 flex flex-col ${leftPanelView === "attachment" ? "opacity-100" : "opacity-0 translate-y-full"}`}
          >
            <div className="p-4 bg-white border-b flex justify-between items-center shadow-sm">
              <span className="text-[10px] font-black uppercase text-slate-500 italic tracking-widest">
                Referral_Slip_Digital.pdf
              </span>
              <div className="flex gap-2">
                <button
                  onClick={handleDownload}
                  className="bg-primary/10 text-primary px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-tight hover:bg-primary/20 transition"
                >
                  Download
                </button>
                <button
...
=======
                <div className="w-full border-b-2 border-slate-800 pb-4 mb-8 text-center font-serif text-xl font-black uppercase italic tracking-tighter">
                  Clinical Referral Slip
                </div>
                <div className="w-full space-y-4 opacity-30">
                  <div className="h-4 bg-slate-200 w-3/4 rounded-full"></div>
                  <div className="h-4 bg-slate-200 w-full rounded-full"></div>
                  <div className="h-24 bg-slate-50 w-full rounded-xl border-2 border-dashed border-slate-100 flex items-center justify-center font-black text-[10px] uppercase tracking-widest italic">
                    Vitals & Case History
                  </div>
>>>>>>> b0420092242b0650ed0d82d71e4793cd88371f53
                </div>
...
=======
          <div
            className={`p-6 flex justify-between items-center transition-all duration-500 ${
              isLog
                ? "bg-slate-50 border-b border-slate-100" // Audit mode: light & clean
                : "bg-gradient-to-r from-emerald-500 to-teal-600" // Active mode: vibrant medical green
            } ${isLog ? "text-slate-800" : "text-white"}`}
          ...
          <div className="p-8 flex-1 overflow-y-auto space-y-8 custom-scrollbar">
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 p-4 bg-slate-50 border border-slate-200 rounded-[1.5rem] flex items-center gap-4 hover:border-primary transition group"
              >
              ...
              <div className="flex-1 p-4 bg-slate-50 border border-slate-200 rounded-[1.5rem] flex items-center gap-4 opacity-40 grayscale cursor-not-allowed">
                <span className="text-2xl">🖼️</span>
                <p className="text-[10px] font-black text-slate-700 uppercase leading-none tracking-tight">
                  Imaging (N/A)
                </p>
>>>>>>> b0420092242b0650ed0d82d71e4793cd88371f53
              </div>
...
=======
              <p className="text-[9px] font-black text-slate-400 uppercase mb-2 tracking-widest leading-none">
                Patient Case Details
              </p>
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight">
                {referral.users?.first_name} {referral.users?.last_name}
              </h3>
              <p className="text-xs italic text-slate-600 mt-2 leading-relaxed font-medium">
...
=======
                  <div className="p-8 bg-slate-50 text-slate-700 rounded-[2.5rem] text-[11px] leading-relaxed border-l-[10px] border-emerald-500 shadow-sm shadow-slate-200/50">
                    ...
                    <div className="space-y-4">
                      {/* Disposition Row */}
                      <div className="flex justify-between items-center border-b border-slate-200/60 pb-3 uppercase">
                        <span className="text-slate-400 tracking-widest font-black">
                          Disposition
                        </span>
                        <b className="text-emerald-600 font-black text-xs tracking-tight">
                          {referral.status}
                        </b>
                      </div>
                      ...
                      {referral.assigned_bed_id && (
                        <div className="flex justify-between items-center pt-2 uppercase">
                          <span className="text-slate-400 tracking-widest font-black italic">
                            Allocated Bed Record
                          </span>
                          <b className="text-primary font-black text-xs tracking-[0.15em]">
                            BED #{referral.assigned_bed_id}
                          </b>
                        </div>
                      )}
                    </div>
                  ...
                  {view === "details" ? (
                    <div className="flex gap-4">
                      <button
                        ...
                        className="flex-1 bg-primary text-white py-5 rounded-[2rem] font-black text-[10px] uppercase tracking-widest shadow-xl shadow-primary/20 hover:scale-[1.02] transition-all"
                      >
                        Accept Case
                      </button>
                      <button
                        onClick={handleDivert}
                        className="flex-1 border-2 border-red-200 text-red-400 py-5 rounded-[2rem] font-black text-[10px] uppercase tracking-widest hover:bg-red-50 hover:border-red-400 transition-all"
                      >
                        Divert Patient
                      </button>
>>>>>>> b0420092242b0650ed0d82d71e4793cd88371f53
...
=======
                        {["OUTPATIENT", "DIAGNOSTIC", "INPATIENT"].map((s) => (
                          <button
                            ...
                            className={`py-3 rounded-xl text-[9px] font-black uppercase border-2 transition-all ${serviceStream === s ? "border-primary bg-primary/5 text-primary" : "border-slate-100 text-slate-300 hover:border-slate-200"}`}
                          >
                            {s}
                          </button>
                        ))}
...
=======
                        ...
                        {serviceStream === "INPATIENT" ? (
                          wardTypes.map((w, i) => (
                            ...
                            >
                              {w.type.toUpperCase()} — {w.available} AVAILABLE
                              BEDS
                            </option>
                          ))
                        ) : serviceStream === "DIAGNOSTIC" ? (
                          diagnostics.map((d) => {
                            const availableCount =
                              d.total_capacity - (d.current_occupied || 0);
                            const isFull = availableCount <= 0;
                      ...
                      {serviceStream === "INPATIENT" && selectedWard && (
                        <div className="space-y-3">
                          <p className="text-[9px] font-black text-slate-400 uppercase text-center tracking-[0.3em]">
                            Physical Bed Allocation
                          </p>
                          <div className="grid grid-cols-5 gap-2 border-t border-slate-50 pt-4 max-h-[140px] overflow-y-auto p-2 custom-scrollbar">
                            {beds.map((b) => (
                              <button
                                ...
                                className={`py-3 rounded-xl text-[10px] font-black border-2 transition-all ${selectedBed === b.id ? "bg-primary border-primary text-white shadow-lg" : "bg-white border-slate-100 text-slate-400 hover:border-primary/30"}`}
                              >
                                {b.bed_label || `#${b.id}`}
                              </button>
                            ))}
...
=======
                        <button
                          ...
                          className="flex-1 bg-slate-100 text-slate-500 py-4 rounded-[1.5rem] font-black text-[10px] uppercase"
                        >
                          Back
                        </button>
                        <button
                          ...
                          disabled={
                            isProcessing ||
                            !selectedWard ||
                            (serviceStream === "INPATIENT" && !selectedBed)
                          }
                          className="flex-[2] bg-primary text-white py-4 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest shadow-xl shadow-primary/20 disabled:opacity-30"
                        >
                          {isProcessing
                            ? "SYNCHRONIZING..."
                            : "FINALIZE & COMMIT"}
